...
module: backup
author: "mengx"
short_description: Backup file or database.
description:
    - Backup file or database, and use ftp upload
options:
    type:
        description:
...
class Database(object):
    ...
    def db_dump(self, module, host, user, password, db_name, target,
                all_database, port, config_file, **kwargs):
        if self.db_type == 'mysql':
            return self._mysql_dump(module, host, user, password, db_name,
                                    target, all_database, port, config_file,
                                    **kwargs)

    def _mysql_dump(self, module, host, user, password, db_name, target,
                    all_databases, port, config_file, socket=None,
...
class Upload(object):
    ...
    def _ftp(self, src, desc):
        ftp = MyFTP(self.host, self.port, self.user, self.passwd, self.timeout,
                    True)
        ftp.upload(src, desc)
        ftp.quit()
...
class MyFTP(ftplib.FTP):
    ...
    def __init__(self, host="", port=21, user="", passwd="", timeout=60,
                 force=False):
        self.host = host
        self.port = port
        self.user = user
    ...
    def upload(self, local_path, remote_path):
        ...
        if rsize < lsize:
            ...
            localf.seek(rsize)
            self.storbinary("STOR " + remotefile, localf, blocksize=1024 * 1024,
                            rest=rsize)
            localf.close()
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            # database params
            db_type=dict(default="mysql", choices=["mysql"]),
    ...
    if backup_type in ("file", "path"):
        ...
        if (dest_len - 1) == dest_position:
            # 指定目录，只指定目录时自动赋值一个名字
            dest_path, dest_filename = dest, "{0}.tar.{1}".format(
                str(int(time.time())), compression)
        else:
            # 指定压缩到文件名  dest 参数中指定
            dest_path, dest_filename = dest[
                                       :dest_position], '{0}-{1}.{2}'.format(
                dest[dest_position + 1:],
                str(int(time.time())), compression)
        ...
        if isinstance(db, list):
            ...
            for d in db:
                ...
                rc, stdout, stderr = database.db_dump(module, db_login_host,
                                                      db_login_user,
                                                      db_login_password, d,
                                                      backup_name,
                                                      all_databases,
            ...
            for dbname, filename in success_db.items():
                compress_filenames.append((filename, '{0}.{1}'.format(
                    filename.split(os.sep)[-1], compression), dest))
            ...
            rc, stdout, stderr = database.db_dump(module, db_login_host,
                                                  db_login_user,
                                                  db_login_password, db,
                                                  backup_name,
                                                  all_databases, db_login_port,
                                                  config_file)
            if rc != 0:
                module.fail_json(msg="%s" % stderr)

            remove_filenames.append(backup_name)
            compress_filenames.append(
                (backup_name, '{0}.{1}'.format(f_name, compression), dest))
    ...
    for u_file, dest_filename, dest_path in upload_filenames:
        ...
        while times:
            try:
                up = Upload(type, login_host, login_port, login_user,
                            login_password)
                # 记录备份文件信息，用于下载
                t_upload_file = "/".join(
                    [dest_path, backnode,
                     datetime.datetime.now().strftime('%Y-%m-%d'), backup_type,
                     dest_filename])
